...
void GWaveView::setPlaybackBuffer(AudioBuffer* buf)
{
  if ( audioBuffer )
  {
    // send old buffer to be de-allocated
    top->getGUI().printTextView( true, "GUI: Deleting old AudioBuffer from waveview widget" );
    cout << "Gui deleting GUI audiobuffer Addr: " << audioBuffer << endl;
    delete audioBuffer;
  }
